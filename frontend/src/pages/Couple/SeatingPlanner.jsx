...
const TableVisualizer = ({ table, onAssignClick, onDeleteClick }) => {
  ...
  return (
    <Card className="flex flex-col items-center p-8 relative hover:shadow-md transition-shadow group" style={{ minHeight: "260px" }}>
      {onDeleteClick && (
        <button 
      ...
      <div style={{ width: "100%", marginTop: "1rem" }}>
        <Text secondary style={{ fontSize: "0.85rem", marginBottom: "8px", display: "block" }}>
          {t("nav_guests")}:
        </Text>
        <div className="flex flex-col gap-2">
          {table.guests?.map((g, i) => {
            ...
            return (
              ...
              >
                <span>{g.fullName || g.name || (g.firstName + " " + g.lastName)}</span>
                <span className="text-xs text-gray-500 font-medium bg-white px-2 py-0.5 rounded-full shadow-sm">{numSeats} {numSeats === 1 ? 'pers' : 'persoane'}</span>
              </div>
            );
          })}
          {!table.guests?.length && (
            <Text secondary style={{ fontStyle: "italic", fontSize: "0.85rem" }}>
              {t("empty_table")}
            </Text>
          )}
        </div>
...
export default function SeatingPlanner() {
  ...
  const handleAddTable = async () => {
    try {
      await api.seating.createTable(EVENT_ID, { name: newTable.name, seats: newTable.capacity });
      setShowAdd(false);
  ...
  const handleDeleteTable = async (tableId) => {
    if (!window.confirm("Ești sigur că vrei să ștergi această masă? Toți invitații de la ea vor rămâne fără loc alocat.")) return;
    try {
  ...
  const totalSeats = tables.reduce((acc, t) => acc + (t.seats || t.capacity || 0), 0);
  ...
  const toggleGuestSelect = (guestId, currentTotalSlots, guestSlots, capacity) => {
  ...
  const handleSaveAssigned = async () => {
    try {
      await api.seating.assignGuests(EVENT_ID, assigningTable.id, selectedGuestIds);
      closeAssignModal();
      loadData(); // reload
    } catch (e) {
      console.error(e);
  ...
  const availableGuests = guests.filter(g => (g.rsvpStatus || "").toLowerCase() !== "declined");
  const uniqueGroups = [...new Set(availableGuests.map(g => g.category || g.group).filter(Boolean))];
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        >
          {tables.map((table, index) => (
            <TableVisualizer key={index} table={table} onAssignClick={openAssignModal} onDeleteClick={handleDeleteTable} />
          ))}
      ...
      {assigningTable && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(0, 0, 0, 0.4)', backdropFilter: 'blur(2px)' }}>
            <Card className="w-full max-w-4xl bg-white flex flex-col shadow-xl" style={{ maxHeight: "85vh", padding: 0 }}>
                {/* Header */}
                <div className="p-6 border-b flex justify-between items-center" style={{ borderColor: 'var(--color-border)' }}>
                    <div>
                        <Heading level={3} style={{ marginBottom: "0.25rem" }}>{t("assign_guests")}</Heading>
                        <Text secondary>Pentru masa: <span style={{ fontWeight: 600, color: 'var(--color-primary)' }}>{assigningTable.name}</span></Text>
                    </div>
                    {(() => {
                        ...
                        return (
                            <div className="text-right">
                                <div style={{ fontSize: "1.25rem", fontWeight: 700, color: overCapacity ? '#ef4444' : 'var(--color-text)' }}>
                                    {currentSlots} <span style={{ fontSize: "1rem", color: "var(--color-text-secondary)", fontWeight: 400 }}>din {capacity} locuri</span>
                                </div>
                            </div>
                        );
                    })()}
                ...
                {/* Filter */}
                <div className="p-4 border-b bg-gray-50/50 flex gap-4 items-center" style={{ borderColor: 'var(--color-border)' }}>
                    <Text secondary style={{ whiteSpace: "nowrap" }}>Filtrează după grup:</Text>
                    <select 
                        value={groupFilter} 
                ...
                <div className="flex-1 overflow-auto">
                    <table className="w-full text-left border-collapse" style={{ minWidth: "600px" }}>
                        <thead className="sticky top-0 bg-white shadow-sm z-10">
                            <tr>
                                <th className="p-4 font-semibold text-sm border-b" style={{ color: "var(--color-text)", borderColor: "var(--color-border)", width: "50px" }}></th>
                                <th className="p-4 font-semibold text-sm border-b" style={{ color: "var(--color-text-secondary)", borderColor: "var(--color-border)" }}>Nume complet</th>
                                <th className="p-4 font-semibold text-sm border-b" style={{ color: "var(--color-text-secondary)", borderColor: "var(--color-border)" }}>Locuri</th>
                                <th className="p-4 font-semibold text-sm border-b" style={{ color: "var(--color-text-secondary)", borderColor: "var(--color-border)" }}>Categorie</th>
                                <th className="p-4 font-semibold text-sm border-b" style={{ color: "var(--color-text-secondary)", borderColor: "var(--color-border)" }}>Status</th>
                            </tr>
                        </thead>
                        <tbody>
                            {availableGuests
                                ...
                                .map(g => {
                                    ...
                                    return (
                                        <tr 
                                            ...
                                            style={{ borderColor: "var(--color-border)", backgroundColor: isSelected ? "rgba(var(--color-primary-rgb, 0,0,0), 0.04)" : undefined }}
                                            onClick={() => {
                                        ...
                                        >
                                            ...
                                            <td className="p-4">
                                                <Badge color={isSelected ? "var(--color-primary)" : "gray"}>{guestSlots} loc{guestSlots !== 1 ? 'uri' : ''}</Badge>
                                            </td>
                                            ...
                                            <td className="p-4">
                                                {isAlreadyElsewhere && !isSelected ? (
                                                    <span style={{ fontSize: "0.75rem", backgroundColor: "#fef3c7", color: "#b45309", padding: "4px 8px", borderRadius: "12px" }}>
                                                        La {existingTable ? existingTable.name : "altă masă"}
                                                    </span>
                                                ) : isSelected ? (
                                                    <span style={{ fontSize: "0.75rem", backgroundColor: "var(--color-primary)", color: "white", padding: "4px 8px", borderRadius: "12px", opacity: 0.8 }}>
                                                        Selectat
                                                    </span>
                                                ) : (
                                                    <span style={{ fontSize: "0.75rem", backgroundColor: "#f1f5f9", color: "#6b7280", padding: "4px 8px", borderRadius: "12px" }}>
                                                        Nepartizat
                                                    </span>
                                                )}
                                            </td>
                ...
                {/* Footer */}
                <div className="p-5 border-t flex justify-end gap-3" style={{ borderColor: 'var(--color-border)', backgroundColor: 'var(--color-bg)' }}>
                    <Button variant="secondary" onClick={closeAssignModal}>{t("btn_cancel")}</Button>
